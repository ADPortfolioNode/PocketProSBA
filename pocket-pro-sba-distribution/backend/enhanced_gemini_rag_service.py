    ...
    def _initialize_vector_store(self) -> bool:
        ...
        try:
            ...
            if embeddings is None:
                # Try falling back to a local SentenceTransformer embedding (ONNX or HF)
                try:
                    import chromadb
                    ...
                    self.logger.info("Falling back to local embedding model: %s", local_model)
                except Exception as fallback_err:
                    raise RuntimeError(
                        f"No working Gemini embedding model found (last error: {last_emb_err}); fallback failed: {fallback_err}"
                    )
