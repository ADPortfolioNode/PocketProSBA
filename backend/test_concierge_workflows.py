...
def test_concierge_basic():
    ...
    try:
        ...
        # Test import
        concierge = Concierge()
        print("✓ Concierge imported and instantiated successfully")
        ...
        for message in test_messages:
            try:
                ...
                if 'text' in result:
                    print(f"✓ '{message[:20]}...' -> Response: {result['text'][:50]}...")
                else:
                    print(f"✗ '{message[:20]}...' -> No text in result: {result}")
            except Exception as e:
                print(f"✗ '{message[:20]}...' -> Error: {e}")
...
def test_rag_integration():
    ...
    try:
        ...
        print(f"✓ RAG Manager instantiated: {rag_manager.is_available()}")
        ...
        if rag_manager.is_available():
            ...
            if "error" in results:
                ...
                print(f"✓ Query '{test_query}' returned {doc_count} documents")
        ...
        for message in test_messages:
            try:
                ...
                if 'response' in result and 'text' in result['response']:
                    print(f"✓ API: '{message[:20]}...' -> Response received")
                else:
                    print(f"✗ API: '{message[:20]}...' -> Invalid response: {result}")
            except Exception as e:
                print(f"✗ API: '{message[:20]}...' -> Error: {e}")
...
if __name__ == "__main__":
    ...
    if all([test1, test2, test3]):
        print("\n🎉 ALL TESTS PASSED! Concierge workflows are working correctly.")
        sys.exit(0)
    else:
